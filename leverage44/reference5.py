"""Independent arithmetic and full-event account audit. No engine/size code imports.

Derived from existing verify_mean account reference, extended for explicitly
frozen 5x budgets and fee-adjusted short sizing. No live liquidation claim.
"""
...
def reference_budget(mode):
    require(mode in MODES,'Unknown reference mode')
    return (D(2) if mode=='BASE' else D(5),
            D(5) if mode=='EXPOSURE5_WHATIF' else D('1.15'),
            D(0) if mode=='EXPOSURE5_WHATIF' else D('3.5'),
            D('.0625') if mode=='EXPOSURE5_WHATIF' else D('.0125'))

def size(cash,p,fee,half,slip,mode,decimals):
    step=D(1).scaleb(-decimals)
    if mode in ('BASE','LEVERAGE5_SAME_SIZE'):
        return (D('10.10')/(p*(1-half))/step).to_integral_value(rounding=ROUND_CEILING)*step
    ep=p*(1-half)*(1-slip); cover=p*(1+half)*(1+slip)
    n=5*cash/(p+5*(max(D(0),p-ep)+(ep+cover)*fee))
...
def audit(row,block,ref):
    ...
    for k,t in enumerate(row['trades'],1):
        ...
        require(stop<=D('.01') and q*ep>=10 and q*ep<=cash*notional_cap,'Stop/notional risk')
        require(q*p0/leverage+q*ep*fee+reserve<=cash and q*ep*(stop+friction)<=cash*risk_cap,'Margin/risk reserve')
        if mode in ('REQUEST5_ORIGINAL_LIMITS','EXPOSURE5_WHATIF'):
            require(q*(p0/5+max(D(0),p0-ep)+(ep+p0*(1+half)*(1+slip))*fee)<=cash,'Post-cost affordability')
        ...
        for off in (2000,21000,40000,59998):
            ...
            if off==2000 and not was_held and first_halt is None and at>row['start_ms']:
                ...
                if eligible:
                    ...
                    st=max(D('.003'),D('1.5')*f['atr']/f['close']);fr=2*(fee+half+slip)
                    should=(qq>0 and nn>=10 and nn<=cash*notional_cap and qq*p/leverage+nn*fee+reserve<=cash
                            and st<=D('.01') and nn*(st+fr)<=cash*risk_cap and st*D('1.8')>=3*fr)
                    if mode in ('REQUEST5_ORIGINAL_LIMITS','EXPOSURE5_WHATIF'):
                        attempt_times.append(at)
                        should=should and qq*(p/5+max(D(0),p-ep)+(ep+p*(1+half)*(1+slip))*fee)<=cash
    ...
    return dict(status='PASS_REFERENCE_SIZE_FULL_ENTRIES_FIRST_EXIT_LEDGER_AND_MARGIN_PROXY',trades=n,funding_events=totalfund,margin_observations=len(margin_buffers))
