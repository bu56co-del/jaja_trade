"""Exit-only reference audit; no production Config, strategy or engine imports.

Extends the existing independent account calculation with separately declared
exit factors; counts are model consistency evidence, not exchange certification.
"""
...
MODES=tuple(LEVELS)
FACTORS={'BASE':(D(1),D(1)), 'SL_HALF':(D('.5'),D(1)),
         'TP_HALF':(D(1),D('.5')), 'BOTH_HALF':(D('.5'),D('.5')),
         'BOTH_QUARTER':(D('.25'),D('.25'))}


def distances(atr, close, profile):
    require(profile in FACTORS, 'Unknown reference exit profile')
    s,t=FACTORS[profile]
    original=max(D('.003'),D('1.5')*D(atr)/D(close))
    return original, original*s, original*D('1.8')*t


def reference_budget(mode):
    require(mode in LEVELS,'Unknown reference mode')
    lev=D(LEVELS[mode])
    return lev,lev,D(0),lev*D('.0125')

def size(cash,p,fee,half,slip,mode,decimals):
    leverage,_,_,_=reference_budget(mode)
    step=D(1).scaleb(-decimals)
    ep=p*(1-half)*(1-slip);cover=p*(1+half)*(1+slip)
    n=leverage*cash/(p+leverage*(max(D(0),p-ep)+(ep+cover)*fee))
    return (n/step).to_integral_value(rounding=ROUND_FLOOR)*step
...
def audit(row,block,ref):
    ...
    mode=row['leverage_mode']; leverage,notional_cap,reserve,risk_cap=reference_budget(mode)
    cfg=row['account_config']
    profile=row['exit_profile'];require(profile in FACTORS,'Unknown exit profile')
    sf,tf=FACTORS[profile]
    for key,val in dict(stop_floor_fraction=D('.003')*sf,stop_ceiling_fraction=D('.01')*sf,
                        atr_multiplier=D('1.5')*sf,reward_to_risk=D('1.8')*tf/sf).items():
    ...
    for k,t in enumerate(row['trades'],1):
        ...
        expected=size(cash,p0,fee,half,slip,mode,decimals)
        eq(q,expected,'Quantity rounding')
        atr=D(t['signal_atr']);c=D(t['signal_close']);original,stop,target=distances(atr,c,profile);friction=2*(fee+half+slip)
        ...
        if mode in LEVELS:
            require(q*(p0/leverage+max(D(0),p0-ep)+(ep+p0*(1+half)*(1+slip))*fee)<=cash,'Post-cost affordability')
    ...
    for b in bs:
        ...
        for off in (2000,21000,40000,59998):
            ...
            if off==2000 and not was_held and first_halt is None and at>row['start_ms']:
                ...
                should=False
                if eligible:
                    qq=size(cash,p,fee,half,slip,mode,decimals)
                    ep=p*(1-half)*(1-slip); nn=qq*ep
                    original,st,tg=distances(f['atr'],f['close'],profile);fr=2*(fee+half+slip)
                    should=(qq>0 and nn>=10 and nn<=cash*notional_cap and qq*p/leverage+nn*fee+reserve<=cash
                            and original<=D('.01') and nn*(st+fr)<=cash*risk_cap and tg>=3*fr)
                    if mode in LEVELS:
                        attempt_times.append(at)
                        should=should and qq*(p/leverage+max(D(0),p-ep)+(ep+p*(1+half)*(1+slip))*fee)<=cash
    ...
    return dict(status='PASS_TIGHT_EXIT_REFERENCE_FULL_ENTRIES_FIRST_EXIT_LEDGER_MARGIN',trades=n,funding_events=totalfund,margin_observations=len(margin_buffers))
