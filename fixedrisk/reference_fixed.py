"""Fixed-risk audit using independently declared caps and chronological accounting.
Derived from the earlier engine-external reference; no production risk/engine imports.
This is self-verification with shared reference ancestry, not third-party certification.
"""
...
PROFILES=('RISK125','RISK125_TP_HALF')


def reference_plan(cash,peak,p,fee,half,slip,atr,close,decimals,profile):
    require(profile in PROFILES,'Unknown reference profile')
    stop=max(D('.003'),D('1.5')*D(atr)/D(close))
    target=stop*(D('.9') if profile=='RISK125_TP_HALF' else D('1.8'))
    ep=p*(1-half)*(1-slip);cover=p*(1+half)*(1+slip)
    friction=2*(fee+half+slip)
    boundary=max(D('9.5'),D('.95')*peak)
    room=max(D(0),cash-boundary-D('.001')*cash)
    budget=min(cash/D(80),room)
    exposure_unit=p/2+ep*fee+max(D(0),p-ep)
    margin_unit=p/5+fee*(ep+cover)+max(D(0),p-ep)
    caps={'RISK':budget/(ep*(stop+friction)),
          'POST_ENTRY_EXPOSURE':cash/exposure_unit,
          'CASH_RESERVE':max(D(0),cash-D('3.50'))/margin_unit}
    smallest=min(caps.values());step=D(1).scaleb(-decimals)
    qty=(smallest//step)*step
    reasons=[]
    if qty<=0:reasons.append('SIZE_ZERO')
    if qty*ep<10:reasons.append('MIN_NOTIONAL')
    if stop>D('.01'):reasons.append('VOLATILITY')
    if target<3*friction:reasons.append('COST_GATE')
    post=cash-qty*(ep*fee+max(D(0),p-ep))
    item=dict(cash=cash,peak_equity=peak,quantity=qty,entry=ep,notional=qty*ep,
         initial_margin=qty*p/5,planned_loss=qty*ep*(stop+friction),
         loss_fraction=qty*ep*(stop+friction)/cash,post_entry_mark_exposure=qty*p/post,
         fee_reserve=qty*cover*fee,risk_budget=budget,drawdown_boundary=boundary,
         remaining_drawdown_budget=room,headroom=cash*D('.001'),quantity_caps=caps,
         binding_caps=[k for k,v in caps.items() if v==smallest],rejected_by=reasons)
    return qty,item
...
def audit(row,block,ref):
    ...
    leverage,notional_cap,reserve,risk_cap=D(5),D(2),D('3.50'),D('.0125')
    cfg=row['account_config']
    profile=row['candidate'];require(profile in PROFILES,'Unknown fixed-risk candidate')
    rr=D('.9') if profile=='RISK125_TP_HALF' else D('1.8')
    require(cfg['risk_profile']==profile and cfg['mode']=='paper_only' and cfg['coin']=='ETH','Profile identity')
    for key,val in dict(initial_balance=D(10),stop_floor_fraction=D('.003'),stop_ceiling_fraction=D('.01'),
                        atr_multiplier=D('1.5'),reward_to_risk=rr).items():
        eq(cfg[key],val,'Frozen exit configuration '+key)
    for key,val in dict(leverage_for_margin=leverage,max_notional_to_equity=notional_cap,minimum_cash_reserve=reserve,risk_fraction_per_trade=risk_cap).items():
        eq(cfg[key],val,'Frozen policy '+key)
    eq(cfg['account_halt_drawdown'],D('.05'),'Account halt retained')
    require(cfg['max_consecutive_losses']==3 and cfg['max_hold_seconds']==21600,'Loss/time policy retained')
    decimals=block['metadata']['szDecimals'] if 'szDecimals' in block['metadata'] else block['metadata']['sz_decimals']
    ...
    for k,t in enumerate(row['trades'],1):
        ...
        atr=D(t['signal_atr']);c=D(t['signal_close']);stop=max(D('.003'),D('1.5')*atr/c)
        original=stop;target=rr*stop;friction=2*(fee+half+slip)
        require(original<=D('.01') and q*ep>=10 and q*ep<=cash*notional_cap,'Stop/notional risk')
        require(q*p0/leverage+q*ep*fee+reserve<=cash and q*ep*(stop+friction)<=cash*risk_cap,'Margin/risk reserve')
        require(q*(p0/leverage+max(D(0),p0-ep)+(ep+p0*(1+half)*(1+slip))*fee)+reserve<=cash,'Post-cost reserve')
        require(q*p0<=2*(cash-q*(ep*fee+max(D(0),p0-ep))),'Post-entry exposure')
        require(target>=3*friction,'Cost gate')
        eq(t['stop'],ep*(1-d*stop),'Stop');eq(t['target'],ep*(1+d*target),'Target')
        eq(t['planned_loss'],q*ep*(stop+friction),'Planned loss');eq(t['initial_margin_model'],q*p0/leverage,'Margin')
    ...
    last_close=None;observed_opens=[];margin_buffers=[];margin_ratios=[];margin_events=[];attempt_times=[]
    attempts={a['time']:a for a in row['sizing_attempts']}
    require(len(attempts)==len(row['sizing_attempts']),'Duplicate sizing attempt')
    for b in bs:
        ...
        for off in (2000,21000,40000,59998):
            ...
            p=px(at);pre=cash;reason=None;was_held=held is not None
            def margin_record(t):
                q=D(t['qty']);entry=D(t['entry']);equity=cash+t['direction']*q*(p-entry)
                maxlev=block['metadata'].get('maxLeverage',block['metadata'].get('max_leverage'))
                maintenance=q*p/(2*D(maxlev));buffer=equity-maintenance
                margin_buffers.append(buffer);margin_ratios.append(equity/(q*p))
                margin_events.append(dict(time=at,trade_id=t['id'],equity=str(equity),notional=str(q*p),maintenance=str(maintenance),buffer=str(buffer)))
                require(buffer>0,'Maintenance breach: exact liquidation unverified')
            if held: margin_record(held)
            if held:
                ...
                if at==held['closed_ms']:
                    ...
                    held=None;last_close=at
            if off==2000 and not was_held and first_halt is None and at>row['start_ms']:
                f=ref[(at-base)//minute]
                eligible=f['direction']==-1 and (last_close is None or at-last_close>=900000) and sum(at-o<86400000 for o in observed_opens)<6
                should=False
                if eligible:
                    peak=max(peak,cash)
                    qq,check=reference_plan(cash,peak,p,fee,half,slip,f['atr'],f['close'],decimals,profile)
                    attempt_times.append(at);require(at in attempts,'Missing sizing attempt')
                    actual=attempts[at]
                    require(actual['rejected_by']==check['rejected_by'],'Rejection reasons')
                    require(actual['binding_caps']==check['binding_caps'],'Binding caps')
                    for key,value in check.items():
                        if key in ('rejected_by','binding_caps'):continue
                        if key=='quantity_caps':
                            require(actual[key].keys()==value.keys(),'Cap keys')
                            for cap,limit in value.items():eq(actual[key][cap],limit,'Quantity cap '+cap)
                        else:eq(actual[key],value,'Sizing field '+key)
                    should=not check['rejected_by']
                    if should:
                        require(at in open_map,'Unearned missing entry')
                        eq(open_map[at]['qty'],qq,'Full chronological quantity')
                        require(D(open_map[at]['planned_loss'])<=check['risk_budget'],'Excess planned budget')
                require(should==(at in open_map),'Missing or unearned entry '+str(at))
            if at in open_map:
                require(held is None and first_halt is None,'Overlap/restart')
                held=open_map[at];cash-=D(held['entry_fee']);observed_opens.append(at);margin_record(held)
            post=cash
            if held:
    ...
    ms=row['margin_diagnostic']
    require(ms['breaches']==0 and ms['observations']==len(margin_buffers),'Margin record count')
    if margin_buffers:
        eq(ms['minimum_buffer_usdc'],min(margin_buffers),'Margin buffer')
        eq(ms['minimum_equity_to_notional'],min(margin_ratios),'Margin ratio')
        idx=margin_buffers.index(min(margin_buffers));actual=ms['minimum_observation'];expected=margin_events[idx]
        require(actual['time']==expected['time'] and actual['trade_id']==expected['trade_id'],'Min margin identity')
        for key in ('equity','notional','maintenance','buffer'):eq(actual[key],expected[key],'Margin event '+key)
    else: require(ms['minimum_buffer_usdc'] is None and ms['minimum_observation'] is None,'No-trade margin')
    require([x['time'] for x in row['sizing_attempts']]==attempt_times,'Attempt chronology')
    return dict(status='PASS_FIXED_RISK_REFERENCE_ENTRIES_BUDGET_EXIT_LEDGER_MARGIN',trades=n,funding_events=totalfund,margin_observations=len(margin_buffers))
