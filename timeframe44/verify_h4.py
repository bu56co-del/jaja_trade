"""Independent 4h geometric EMA and chronological account audit. No engine import."""
from decimal import Decimal as D,ROUND_CEILING
from bisect import bisect_right
from collections import Counter
from reference_alt import require,eq,close_feature
from reference_bb import ma_last

def reference_context(rows,at):
    prior=[b for b in rows if b['T']<at][-120:]
    require(len(prior)==120,'Reference warmup')
    require(at-prior[-1]['T']<=14400000,'Reference stale history')
    c=[D(b['c']) for b in prior]
    tr=[max(D(prior[i]['h']),c[i-1])-min(D(prior[i]['l']),c[i-1]) for i in range(1,120)]
    f=ma_last(c,9)
    return dict(last_closed_ms=prior[-1]['T'],fast=f,slow=ma_last(c,21),rise1=f-ma_last(c[:-1],9),rise4=f-ma_last(c[:-4],9),atr=sum(tr[-14:])/14)

def reference_choices(block,s,base,rows):
    result={};events=[];eps=D('1e-12')
    for j,r in base.items():
        d=r['direction'];ok=True
        if d:
            at=block['candles'][j]['t'];f=reference_context(rows,at)
            mode=s['trend']
            if mode=='UP_VETO':ok=not(f['fast']-f['slow']>eps and f['rise4']-f['atr']/2>eps)
            elif mode=='DOWN_ONLY':ok=f['fast']-f['slow']<=eps and f['rise1']<=eps
            else:require(mode=='NONE','Reference filter')
            events.append(dict(index=j,decision_ms=at,allowed=ok,context={k:str(v) for k,v in f.items()}))
            if not ok:d=0
        close_short=False
        if s['exit']=='MID15' and j%15==0:
            prices=[D(block['candles'][k-1]['c']) for k in range(j-285,j+1,15)]
            require(len(prices)==20,'Reference middle length')
            close_short=D(block['candles'][j-1]['c'])*20<=sum(prices)
        result[j]=dict(r,direction=d,exit_long=False,exit_short=close_short)
    return result,events

def check(a,b,ev,ref):
    require(a.keys()==b.keys(),'Decision keys')
    for j in a:
        for k in ('direction','exit_long','exit_short'):require(a[j][k]==b[j][k],f'Decision mismatch {j} {k}')
        close_feature(a[j]['atr'],b[j]['atr'],'ATR');eq(a[j]['close'],b[j]['close'],'Close');close_feature(a[j]['trace'],b[j]['trace'],'Trace')
    require(len(ev)==len(ref),'Event count')
    for x,y in zip(ev,ref):
        for k in ('index','decision_ms','allowed'):require(x[k]==y[k],'H4 decision mismatch')
        close_feature(x['context'],y['context'],'H4 independent context')
...
def audit(row,block,ref):
    # This auditor preserves the prior formulas and explicitly parameterizes max hold.
    require(row['account_config']['max_hold_seconds'] in (14400,21600),'Frozen hold duration')
    ...
    for b in bs:
        ...
        for off in (2000,21000,40000,59998):
            ...
            if held:
                ...
                elif at-held['opened_ms']>=row['account_config']['max_hold_seconds']*1000:reason='MAX_HOLD_TIME'
